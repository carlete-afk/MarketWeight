...
public class RepoUsuarioTest : TestBase
{
    ...
    [Fact]
    public void TransferenciaAsyncFAIL()
    {
        var error = Assert.ThrowsAnyAsync<Exception>(() => _repo.TransferenciaAsync(2, 0.5m, 8, 6));
        Assert.Equal("Cantidad Insuficiente!", error.Result.Message);
    }
